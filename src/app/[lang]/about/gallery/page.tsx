...
export default async function GalleryPage({ params, searchParams }: PageProps<'/[lang]/about/gallery'>) {
  const { lang } = await params;
  const searchParamsValue = await searchParams;
  const startAfter = getSingleSearchParam(searchParamsValue.startAfter);
  const albumsResponse = await getAllAlbums(startAfter);
  ...
  return (
    ...
    >
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute top-[38%] left-[20%] w-8 h-8 bg-linear-to-br from-yellow-300/20 to-orange-300/10 dark:from-yellow-500/15 dark:to-orange-500/10 animate-pulse-fade"
      ...
      <div className="container mx-auto relative z-10">
        ...
        <p className="mb-4 text-sm font-medium text-gray-600 dark:text-gray-400">
          Page {albumsResponse.page} of {albumsResponse.totalPages}
        </p>

        <AlbumsList albumList={albumsResponse.albums} previewImages={previewImages} />

        {(albumsResponse.hasPrev || albumsResponse.hasMore) && (
          <div className="mt-8">
            <SimplePagination
              lang={lang}
              prevPage={
                albumsResponse.hasPrev
                  ? buildCursorUrl("/about/gallery", albumsResponse.prevStartAfter)
                  ?? "/about/gallery"
                  : undefined
              }
              nextPage={buildCursorUrl("/about/gallery", albumsResponse.nextStartAfter)}
            />
          </div>
        )}
      </div>
