...
export const getLifeMilestones = cache(async (): Promise<LifeEvent[]> => {
    // 1. Get today's date once for the whole loop
    const today = new Date();

    // 2. Map over your raw data
    const processedEvents = lifeEvents.map((event) => {
        ...
        // 3. Check if the exact anniversary date has passed THIS year
        const hasAnniversaryPassedThisYear =
            today.getMonth() > eventDate.getMonth() ||
            (today.getMonth() === eventDate.getMonth() && today.getDate() >= eventDate.getDate());

        // If it hasn't happened yet this year, subtract 1 from the total years
        if (!hasAnniversaryPassedThisYear) {
            yearsDiff--;
        }

        // 4. Format the output string gracefully
        let timeString = "";

        if (yearsDiff < 0) {
            timeString = "Upcoming"; // Catch for future events like your 2026 ones
        } else if (yearsDiff === 0) {
            timeString = "Less than a year";
        } else if (yearsDiff === 1) {
            timeString = "1 Year";
        } else {
            timeString = `${yearsDiff} Years`; // Handles plurals (e.g., "2 Years")
        }

        // 5. Replace the <time> tag in the title with our calculated string
